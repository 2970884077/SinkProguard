...
public class SimpleNameFactory implements NameFactory
{
    ...
    public SimpleNameFactory(boolean generateMixedCaseNames)
    {
		...
		try
		{
			mDexClassLoader=new DexClassLoader(Constant.MAIN_PATH+"sink.dex","/data/data/com.Sink.ProGuard/cache/",null,DexClassLoader.getSystemClassLoader());
			Class<?> mClass = mDexClassLoader.loadClass("Main");
            mName = mClass.getMethod("name",int.class);
		}
		catch (Exception e)
		{
			e.printStackTrace();
